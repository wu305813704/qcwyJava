var app = angular.module("operation.userDaily",[]);
app.controller("controllerUserDaily",["$scope","$http","utilService",function ($scope,$http,utilService) {
//&pageNum=1&pageSize=2
    ...
    $scope.url = "http://192.168.3.108:8080/qcwy/bg/userLog?token="+localStorage.token+"&type=0&pageNum="+$scope.index+"&pageSize=5";
    ...
    $("#pageLftUp").on("click",function () {
        ...
        $scope.url = $scope.baseURL+"bg/userLog?token="+localStorage.token+"&type=0&pageNum="+$scope.index+"&pageSize=5";
        utilService.requestData($scope);
    });
    //点击上一页
