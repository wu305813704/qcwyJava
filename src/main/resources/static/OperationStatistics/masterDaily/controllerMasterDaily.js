var app = angular.module("operation.masterDaily",[]);
app.controller("controllerMasterDaily",["$scope","$http","utilService",function ($scope,$http,utilService) {
    $scope.baseURL = utilService.baseURL();
    $scope.index == undefined? $scope.index = 1:$scope.index= sessionStorage.index;
    $scope.url = $scope.baseURL+"bg/userLog?token="+localStorage.token+"&type=1&pageNum="+$scope.index+"&pageSize=5";
